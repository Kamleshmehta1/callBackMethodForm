class Table {
  constructor(tableContainerId, storageId, formContainerId) {
    this.tr;
    this.headTr;
    this.globalArr = [];
    this.td;
  ...
  renderTable(data, cb) {
    ...
    data != null ?
      data.forEach((Obj, idx) => {
        ...
        editButton.onclick = (event) => this.handleEdit(event, Obj.userId, idx, cb);

        deleteButton.classList = "button delete";
        deleteButton.onclick = (event) => this.handleDelete(event, Obj.userId, cb);
  ...
  handleEdit(e, userId, idx, cb) {
    ...
    submitNode.nextSibling["onclick"] = (event) => {
      ...
      this.handleUpdate(e, userId, idx, cb);
    };
  }
  handleUpdate(e, userId, index, cb) {
    e.preventDefault();
    console.log("UPDATE");
    let globalObject = {};
    let currentNode = Array.from(e.target.parentNode.parentNode.parentNode.childNodes);
    ...
    let idx = this.getLocalStorage().findIndex((ele) => ele.userId === userId)
    updateArr[idx] = globalObject;
